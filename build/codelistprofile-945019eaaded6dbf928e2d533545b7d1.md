...
- **JSON:**
  ```json
  "skos:hasTopConcept": [
    {"@id": "sf:anysampledfeature", "@type": ["skos:Concept"], "...": "..."}
  ]
  ```
...
- **JSON / Scope note:**
  ...
  - `skos:notation` — classification code(s) for the scheme.
  - Documentation notes: `skos:note`, `skos:scopeNote`, `skos:historyNote`, `skos:changeNote`, `skos:editorialNote`, `skos:example`.
...
- **Scope note:** `altLabel` carries alternative labels (acronyms, abbreviations, spelling variants); `hiddenLabel` carries labels accessible to free-text search but not displayed.

### Concept mappings
- **Obligation:** optional, repeatable
- **JSON:** `"skos:exactMatch": [{"@id": "{concept-uri}"}]` (same form for the others)
- **Scope note:** Links to concepts in **other** schemes: `skos:exactMatch` (equivalent), `skos:closeMatch` (similar), `skos:broadMatch`, `skos:narrowMatch`, `skos:relatedMatch`. Use `skos:related` for associatively related concepts **within** the same scheme.

### Concept documentation notes
- **Obligation:** optional
- **JSON:** `"skos:scopeNote": "{string or LanguageTaggedValue}"`
- **Scope note:** `skos:note`, `skos:scopeNote`, `skos:historyNote`, `skos:changeNote`, `skos:editorialNote`, and `skos:example` provide editorial and usage documentation for the concept.

## Data types

(languagetaggedvalue)=
### LanguageTaggedValue
- **JSON:** `{"@value": "Sampled Feature Type vocabulary", "@language": "en"}`
...
(object-reference)=
### Object reference
- **JSON:** `{"@id": "https://w3id.org/isample/vocabulary/sampledfeature/anysampledfeature"}`
- **Scope note:** A reference to another node by its `@id`, used to link to concepts or schemes defined elsewhere in the graph or externally.
