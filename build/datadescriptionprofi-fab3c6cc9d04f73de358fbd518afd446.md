...
(object-reference)=
## Object References
In the implementation notes 'object reference' indicates that a property value can be specified with a URI. In JSON-LD, this is implemented thus:
```{"\@id":"uri for target"}```
The string that is the value of the the @id is expected to be a resolvable identifier that will get a JSON object of the type expected by the property value definition.  For some metadata elements this will be an object in the same document and the object reference target is the @id node identifier.  The referenced object might exist as an external resource in which case the uri for the target should be a resolvable http uri. 